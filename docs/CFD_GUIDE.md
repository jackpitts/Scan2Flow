...
“CFD-ready” is a design objective: dimensionally checked geometry with a documented route to a suitable fluid domain and mesh. A STEP or STL file alone does not establish the correctness of a simulation. Start with the [system and CLI reference](../DOCUMENTATION.md) and the [ML pipeline](ML_PIPELINE.md) for reconstruction assumptions.
...
The proposed [reconstruction report](../schemas/reconstruction-report.schema.json) records the decision as `accepted_for_cfd_preparation`, `needs_review`, or `rejected`. Its individual checks use `pass`, `fail`, `not_run`, or `unknown`; unavailable metric values are `null`. Acceptance requires the declared geometry evidence and review. Fluid-domain acceptance is a separate downstream decision.
...
These are planned fields, not a currently implemented sidecar schema. Freeze a versioned schema before backend development and document it alongside the actual reader/writer.
...
The complete option table is in [DOCUMENTATION.md](../DOCUMENTATION.md). These examples are syntax-valid planning commands:
